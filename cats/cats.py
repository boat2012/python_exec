...
def main():
    ...
    while ic < Crown*5:
        ...
        os.system('adb shell input tap 1390 957')  #快速
        time.sleep(WAIT_TIME)       # print("第",ic,"场   ",end=' ')
        try:
            # pull_screenshot()
...
def judge():  #判断是否可以胜出
    print("start get picture",end="")
    pull_screenshot()
    image = Image.open(filepath+"1.png")
    llife=(790,114,954,179)
    rlife=(970,114,1150,182)
    lattack = (790,196,954,257)
    rattack=(970,196,1150,257)
    i_their_life = get_rect(image,rlife)
    i_llife=get_rect(image,llife)
    i_lattack=get_rect(image,lattack)
    i_rattack=get_rect(image,rattack)
    print("%d,%d,%d,%d,%.2f,%.2f" % (i_llife,i_their_life,i_lattack,i_rattack,i_their_life/i_lattack,i_llife/i_rattack))
    if i_their_life/i_lattack < i_llife/i_rattack :
        return True
    else:
        return False
